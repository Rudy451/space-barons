...
export class Controller {
  ...
  updateGame(@SocketIO() io: Server, @ConnectedSocket() socket: Socket, @MessageBody() message: any){
    const roomId = Array.from(socket.rooms.values()).filter(player => player != socket.id)[0];
    const targetRoom = Array.from(io.sockets.adapter.rooms).filter((rm:any) => rm.includes(roomId));
    if(targetRoom.length === 1){
      const targetPlayer = Array.from(targetRoom[0][1]).filter(sockId => sockId != socket.id);
      socket.to(targetPlayer).emit('new_game_update', message);
    } else {
  ...
  endGame(@SocketIO() io: Server, @ConnectedSocket() socket: Socket, @MessageBody() message:any){
    ...
    const targetRoom = Array.from(io.sockets.adapter.rooms).filter((rm:any) => rm.includes(roomId));
    const targetPlayer = Array.from(targetRoom[0][1]).filter(sockId => sockId != socket.id);
    socket.emit('game_is_over', message[0], targetRoom[0][0])
    socket.to(targetPlayer).emit('game_is_over', !message[0], targetRoom[0][0]);
  }
}
